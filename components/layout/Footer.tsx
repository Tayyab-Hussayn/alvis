...
/*
 * Derived from `data/services.ts` rather than hardcoded. The previous duplicate
 * list drifted out of sync with the real offering and had to be corrected by hand.
 */
const serviceLinks = services.map(s => s.title)

const socials = [
  { label: 'Facebook',  path: 'M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z', href: socialLinks.facebook },
  { label: 'Instagram', path: 'M7 2h10a5 5 0 0 1 5 5v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a5 5 0 0 1 5-5zm5 5.5a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9zM17.8 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2z', href: socialLinks.instagram },
  { label: 'LinkedIn',  path: 'M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-4 0v7h-4v-7a6 6 0 0 1 6-6zM2 9h4v12H2z M4 2a2 2 0 1 1 0 4 2 2 0 0 1 0-4z', href: socialLinks.linkedin },
]
 ...
 *
 * This previously rendered a bare `<form>` with no submit handler, so the button
 * fired a native GET, reloaded the page and discarded the address — on every page
 * of the site. It now posts to the same endpoint as the blog signup.
 */
function NewsletterForm() {
  const { email, setEmail, state, error, subscribe } = useNewsletterSubscribe('footer')

  if (state === 'done') {
    return (
      <p className="text-[13px] font-bold" style={{ color: '#b7102a' }} role="status">
  ...
  return (
    <form className="relative" onSubmit={subscribe} noValidate>
      ...
      <input
        ...
        className="w-full rounded-full bg-white pl-5 pr-14 py-3 text-[13px] outline-none focus:ring-2 focus:ring-[#e63946]/40"
      ...
      {state === 'error' && (
        <p className="mt-2 text-[12px]" style={{ color: '#b7102a' }} role="alert">{error}</p>
      )}
    </form>
  )
}
      ...
      <div className="max-w-[1280px] mx-auto">
              ...
              {contact.map(c => (
                <li key={c.lines[0]} className="flex items-start gap-3">
                  ...
                  <a
                    ...
                    className="text-[13px] transition-colors hover:text-[#e63946]"
                    style={{ lineHeight: '1.5', color: '#5b403f' }}
                  >
        ...
        >
          <p className="text-[12px]" style={{ color: '#8f6f6e' }}>
            © {new Date().getFullYear()} ALVIS Marketing Agency. All Rights Reserved.
          </p>
